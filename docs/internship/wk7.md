...
> 周总结：重构项目结构，统一异常与 REST 规范，并补充 CI 和安全流程。
...
- 整理项目根目录
  ...
  - 意思是根目录创建docs然后docs里面再创建一个internship
...
- ApiErrorResponse规定了JSON的一个统一结构
...
- 1 FolderService中现存问题
  ...
  - 对folder Service进行修改
  - 让逻辑匹配：id或request为null返回400，数据库中找不到资源返回404
...
- 3 把@Valid校验失败统一成ApiErrorResponse
  - 在GlobalHandler增加处理方法针对于在Controller中,前端JSON先被转成了DTO 随后Spring再执行的校验 
  - @Valid校验失败 抛出MethodArgumentNotValidException 不会再调出Service
  ...
  - 然后用verifyNoInteractions验证校验失败后Service有没有被调用 没有调用就是正常的
...
## 四、GitHub Actions 自动化测试

### 9 增加GitHubAction自动测试
- 在根目录创建workflow文件
...
- 有了workflow之后
  - git push之后 github会自动开一个Ubuntu虚拟机
  - → 安装Java 21 
  - → 启动临时MySql
  - → 创建fileapi数据库
  - → 运行db/schema.sql
...
登录成功了以后
- 进入JWT
- Controller调用TokenService生成JWT Token，返回一个由前端保存的令牌
