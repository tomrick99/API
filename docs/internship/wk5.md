...
### 5 总结:
	...
	4 增加了参数效验
	用户传的JSON不能直接无条件进入数据库 要先经过DTO效验
...
@Valid表示转成对象之后 立即检查这个DTO上的效验规则  
	
	eg:
	@NotBlank(message=...)  
	如果前端传的JSON是不符合注解的 请求直接会变成400 Bad Request  

所以Controller不负责正真的业务, 他负责接请求, 参数和基础效验 然后把请求交给Service
...
2 参数绑定和基础效验
	...
	效验发生在进入方法调用service之前  
	@Valid @RequestBody MaterialCreateReqDTO request  
	@R: JSON转成MaterialCreateReqDTO类型 然后@V 检查DTO里的效验注解  
	eg: DTO里有@NotBlank 传入了空值 效验失败Spring直接返回400   
	不会执行materialService.createMaterial(request)  
	测试里也是 如果效验没过Service根本没被碰到 
...
2 ServiceImpl负责正真执行逻辑  
&emsp;&emsp; 	@Service表示交给Spring管理  
&emsp;&emsp; 	implements 接口类表示实现了接口里面规定的所有方法  
	
3 例子  
&emsp;&emsp;	Controller这一步 return materialService.createMaterial(request);  
&emsp;&emsp;	进入materialService.createMaterial()后  
&emsp;&emsp;	创建时间->  
&emsp;&emsp;	接收MaterialCreateReqDTO->  
&emsp;&emsp;	创建MaterialDO->  
&emsp;&emsp;	复制请求数据->  
&emsp;&emsp;	调用Mapper插入数据库  
&emsp;&emsp;	把DO转成DTO 返回给Controller  
  ...
  为什么还要再转回DO  
  因为DO可能由前端不该看到的字段比如说生成时间 不然客服端就可能会仿造一个request 返回成DTO只返回接口运行公开的数据
	
4 注意Controller调用的是接口 不是业务类  
&emsp;&emsp;	因为Controller只关心要一个能创建,查询,修改资料的业务服务  
&emsp;&emsp;	这个服务由哪一个实现类提供 由Spring负责
...
```
这一句 因为在MaterialMapper里面没有新声明insert 而是继承了BaseMapper<MaterialDO>

由于用了泛型<>所以int inset(T entity)里的T就是MaterialDO  
所以在ServiceImpl里面:  
```java
...
Spring会一位是未处理异常 最后通常会返回HTTP 500
...
1 title为空@Valid效验失败/客户端请求参数不合法 返回400 Bad Request
...
一 pom.xml加入SpringBootStaterSecurity

二 再根目录新加securityConfig文件夹  
- @EnableWebSecurity开启安保  
- authenticated() 要求用户已经完成了身份核验  
- Token过滤器 负责拿证件 验真伪 并且登记来访客身份  
	
三 HttpSecurity http不是某一次用户请求的HTTP对象 而是组装安全规则的工具

首先Spring调用这个方法 得到一个SecurityFilterChain对象 把这个对象注册成一个Bean 以后收到请求的时候使用这一条过滤链

CSRF: 是证明“这个操作来自正常网站页面”的一次性/随机校验码,证明请求不是其他恶意网站诱导浏览器发出的  
	攻击者无法直接获取你的账号 但是能借用你的登录状态执行操作  
	浏览器会自动携带用户的Cookie 关闭的话攻击者用这个行为伪造发送请求 然后客户端以为这个请求是这个用户发的
	

STATELESS: 每一次受保护的业务请求都要携带 Token。 让服务端检查, SrpingSecutity不使用HTTP Session Cookie判断用户是否登录
...
四 @RestController告诉Spring这是REST接口类 方法返回值直接写入HTTP响应
	@RequestMapping("/auth")这个类里的所有路径都是/auth
	
五 在securityConfig里面增加密码处理工具: 哈希(不可解密的加密)
...
八 pow加入JWT依赖 前端拿到的Token就是JWT; 增加Server类 类里在项目启动的时候创建一个签名的钥匙
	期限两个小时 只有用户调用的的时候才用得到

九 签名不等于加密 JWT中的用户名等Payload信息可以被解码查看 知识攻击者修改后无法通过签名验证 所以不要把密码放进JWT
...
十一 新增JWT认证过滤器 继承了OncePerRequestFilter 意思是每一个HTTP请求进过一次 都要过一次这个过滤器  
	然后去掉Header里面的Authorization 和 Bearer 得到JWT TokenService验证签名时间 获得用户名->  
	UserDetialsService查询这个用户是否还存在 并且获得用户权限->  
...
每一次发送HTTP请求都会走一次这个过滤器 然后把整个对象放到securityContext里面 因为config里面有STATELESS
	...
	完整的过程:
		...
		Filter读取JWT ->
		验证JWT ->
		创建Authentication 放入SercrityContext->
		Spring根据授权检查和Controller使用(user-User,admin->Admin)->
		请求结束后再清理context
		
十三 增加管理员接口 然后在securityConfig里面去增加一个管理员访问规则  
...
十四 增加MethodSecurityController 路径不同 请求通过URL 调用方法前再次检查角色
...
十五 currentUserProvider从SecurityContext获取当前用户名  
...
十六 新增LoginUser用来保存用户ID name password authorities  
...
十八 在SecurityConfig里面增加tenant-id的逻辑  
