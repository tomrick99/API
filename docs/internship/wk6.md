...
- Docker里面的redis和本地的redis, 他们运行的是同一个Redis软件 但是运行环境不一样
...
- 表示的是key还能存活多久 单位是秒
  - TTL是查看过期时间的命令:
  - 返回-1: key存在，但是没有设置过期时间
  - 返回-2: key不存在，可能已经过期或被删除
  - 返回正整数: 还剩多少秒过期
...
- 启动Nacos之后可以访问http://127.0.0.1:8848/nacos
...
- 第一步 在MySQL里创建material_folder表，一行代表一个文件夹
  - 问题:
...
- 当前链路仍然存在的问题:
- 1 不存在的ID 非法的ID 参数校验失败的时候接口错误返回:message:请先登录
- 2 更新的时候如果没有传入description 返回JSON的description会是null 但是数据库仍然还是保留旧值 
- 3 仓库里面的schema.sql没有这个项目(material_folder)的建表语句  本机已经有了建好的表 
- 但是换一台电脑或者新建数据库Folder会失败
- 4 没有Folder的测试
- 5 只有按ID查询 没有GET /api/folders 文件夹列表接口
- 6 insert, updateById, deleteById的受影响行数没有判断 极端并发或数据库写入失败时 接口可能仍然返回成功
- 7 REST语义:
  - 创建当前返回200 常见规范为201
...
- 3 换一个方法 不用MockMVC:不在测试里面拼整个JSON
  ...
  - 后面controller调用这个service就会返回一个成功且对应的response 不是去执行真正的业务
